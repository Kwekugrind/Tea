...
const SYMBOL = "R_25";
const SYMBOL_NAME = "Volatility 25 Index";
const REPO_LABEL = "Tea Machine";
// ==================================================================
        ...
        if (!settledResult) {
          ...
          if (macdFlipped) {
            if (!openTrade.macdEarlyFlipEpoch) {
              ...
              exitReason = inProfit ? "MACD(8,100) Trail Exit — held above entry" : "MACD(4,34) Early Exit — price below entry";
            }
          } else {
            if (openTrade.macdEarlyFlipEpoch) { openTrade.macdEarlyFlipEpoch = null; fs.writeFileSync("trades.json", JSON.stringify(trades, null, 2)); }
          }
        }
      }
